...
- `team_season_score_archive` ← `insert ... select id, season_score from teams where season_score is not null` (6 rows: the soft-deleted `test` team included — the INSERT has no `deleted_at` filter; verified 2026-09-04).
...
**Players admin (2026-09-05).** `GET /api/players` and `GET /api/players/[id]` attach `season_stats` (the ACTIVE season row of `player_season_stats`; null = no appearances) plus `active_season`; the `/dashboard/players` cards and the editor drawer show those numbers read-only and the hand-typed goals/assists/cards inputs are gone (`PlayerFormPayload` no longer carries them; the APIs still accept them for the untouched teams-dashboard PlayerBoard). The legacy `player_statistics` join survives only for `age`. Helper: `lib/activeSeasonPlayerStats.ts`.
...
**Roster re-signing (2026-09-15).** The teams-dashboard "Προσθήκη υπάρχοντος" drawer (`dashboard/teams/PlayersPanel.tsx`) searches as the admin types via `GET /api/players/search` (accent/case/final-sigma folded, word-prefix ranked in JS by `lib/playerSearch.ts`; each hit lists the ACTIVE-season teams the player is already on) and shows "Παίκτες από την προηγούμενη σεζόν" from `GET /api/teams/[id]/previous-roster`: the roster of the row this team continues (`teams.copied_from_team_id`, else the same name in the newest other season, flagged `via: "name"`), minus players already on this season's row, each with their source-season `player_season_stats` numbers. "Προσθήκη" / "Προσθήκη όλων" go through the existing `POST /api/teams/[id]/players` — a new `player_teams` row on the active-season team; the old season's row is untouched (D1).
...
## 3.10 Hardening — written 2026-09-04 (after the neutral review of the branch)

**A · integrity.** `tournaments.season` is validated on the save-all edit path (`resolveSeasonLabel` in `lib/seasons.ts`, never null); every team on a tournament or in its matches must be a row of the tournament's season (`assertTeamsInSeason`, D1) — enforced by save-all, the create action and the match POST/PATCH routes; a season MOVE re-snapshots both seasons and the wizard warns first. Match and tournament deletes refresh `player_season_stats` / `player_tournament_stats` for the affected players. Disciplinary adjustments, postponements and the create action refresh the stored standings like every other site. `POST /api/teams` creates rows in the active season only. The points engine is split into a pure `computeStandingsFromInputs` (`geniki-katataxi/engine.ts`, unit-tested) and a loader that reads one season's rows; the date-derived `SeasonMode` is gone.

**B · close flow.** The pointer flips in ONE transaction — `flip_active_season` / `set_active_season` in `migrations/add-season-flip-fn.sql` (service-role only; run BEFORE deploying the code) — so readers never see zero active rows; reopen stamps `ended_on` too. The actions refuse to run on the Vercel preview (shared prod DB) unless `ALLOW_SEASON_POINTER_ON_PREVIEW=1`. `revalidateSeasonSurfaces` drops every cached team/tournament page, every nested archive page and the sitemap; the archive redirects are decided by the row's own season status, on all four tournament/team routes; the sitemap lists archived rows under `/seasons`.

**C · public consistency.** One rank rule (`standingsShape.rankVisible`) for the live table, the archive and the team page. Home video strip and recap are pointer-driven (recap: active season, else the newest archived season's stored `season_recaps` payload). `/paiktes` ignores a `?tournament_id=` outside the active season. Missing team → real 404.
