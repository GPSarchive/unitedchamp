...
/* ======================================
   ...
   Request body:
   {
     new_match_date?: string (ISO 8601, optional - can postpone without setting a new date),
     postponement_reason?: string (optional - can postpone without a reason)
   }
   ====================================== */
export async function POST(
  ...
  try {
    ...
    const postponementUpdate = {
      ...
      match_date: newDateISO ?? null, // Keep existing date or set to null
    ...
    // Calculate announcement end date (show until 1 day after new match date, or 7 days if no date set)
    const endDate = newDate ? new Date(newDate) : new Date();
    if (newDate) {
      endDate.setDate(endDate.getDate() + 1);
    } else {
      endDate.setDate(endDate.getDate() + 7); // Show for 7 days if no new date set
    }
    ...
    const successMessage = newDateFormatted
      ? `Match postponed successfully from ${oldDateFormatted} to ${newDateFormatted}`
      : `Match postponed successfully. Original date: ${oldDateFormatted}`;
