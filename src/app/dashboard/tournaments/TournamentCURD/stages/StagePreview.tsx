//app/dashboard/tournaments/TournamentCURD/stages/StagePreview.tsx
"use client";
...
function StageFlowChips({ payload, stageIdx }: { payload: NewTournamentPayload; stageIdx: number }) {
  ...
  // who do I feed into?
  const output = (() => {
    const nextKO = payload.stages.find(
      (s: any) => s?.kind === "knockout" && (s?.config?.from_stage_idx ?? -1) === stageIdx
    ) as any;

    if (!nextKO) return null;

    // Source kind is this stage's kind (the one feeding KO)
    const srcKind = (payload.stages[stageIdx] as any)?.kind as "groups" | "league" | "knockout" | undefined;
    const koCfg = (nextKO?.config ?? {}) as any;

    if (srcKind === "groups") {
      const adv = koCfg?.advancers_per_group ?? 2;
      const cross = koCfg?.semis_cross ?? "A1-B2";
      return `→ KO: ${nextKO.name} • ${adv}/όμιλο • ${cross}`;
    }

    if (srcKind === "league") {
      // Prefer advancers_total; fallback to advancers_per_group; then a safe default
      const advTotal =
        Number.isFinite(koCfg?.advancers_total) ? koCfg.advancers_total
        : Number.isFinite(koCfg?.advancers_per_group) ? koCfg.advancers_per_group
        : 4;
      return `→ KO: ${nextKO.name} • top ${advTotal} (by seed)`;
    }

    // Fallback
    return `→ KO: ${nextKO.name}`;
  })();
...
function KnockoutPreview({
  ...
  // Keep original indices for write-back, but ensure view ids match (round, bracket_pos) order
      ...
      draftMatches
        ...
        .sort(
          (a, b) =>
            ...
            (a.m.bracket_pos ?? 0) - (b.m.bracket_pos ?? 0)
        ),
    [draftMatches, stageIdx]
  );

  const viewToOrig = useMemo(() => {
    const map = new Map<number, number>();
    entries.forEach((e, i) => map.set(i + 1, e.idx));
    return map;
  }, [entries]);
  ...
  const viewIdToOrigIdx = (viewId: number): number | null => {
    return viewToOrig.get(viewId) ?? null;
  };
  ...
  return (
    <div className="space-y-3">
      <ModernKnockoutTree
        ...
        teamsMap={teamsMap}
        editable={false}
        // ⬇️ give the tree some air
        colWidth={300}
        gapX={356}
        minCardHeight={104}   // make every card the same height
        minRowGap={28}        // more vertical air so the clamp doesn't move parents
