...
const selDbOverlayBySig = (s: TournamentState) => s.dbOverlayBySig as Record<string, Partial<DraftMatch> & { db_id?: number | null; updated_at?: string | null }>;
...
const selUpdateMatches = (s: TournamentState) => s.updateMatches as (stageIdx: number, recipe: (stageRows: DraftMatch[]) => DraftMatch[]) => void;
const selListGroupTeamIds = (s: TournamentState) => s.listGroupTeamIds as (stageIdx: number, groupIdx: number) => number[];
...
function safeOverlay(ov?: Partial<DraftMatch> & { db_id?: number | null; updated_at?: string | null }) {
  ...
  const { match_date, stageIdx, groupIdx, matchday, round, bracket_pos, team_a_id, team_b_id, ...rest } = ov as any;
...
/* ---------------- component ---------------- */
export default function InlineMatchPlanner({ miniPayload, teams, forceStageIdx }: {
  ...
  useEffect(() => {
    ...
    return () => { if ((window as any).useTournamentStore === useTournamentStore) delete (window as any).useTournamentStore; };
  }, []);

  const nameOf = useMemo(() => {
    const local = new Map<number, { name: string; logo: string | null }>();
    (teams ?? []).forEach((t) => {
      if (t && typeof t.id === "number") local.set(t.id, { name: t.name ?? `Team #${t.id}`, logo: t.logo ?? null });
    });
  ...
  const storeGroups = useMemo(() => {
    return Object.keys(normalizedGroupMap).length
      ? Object.keys(normalizedGroupMap).map((idxStr) => ({ idx: Number(idxStr), id: normalizedGroupMap[Number(idxStr)] }))
          .sort((a, b) => a.idx - b.idx).map(({ idx, id }) => ({ idx, id, name: (groupsById as any)?.[id]?.name ?? `Group ${idx + 1}` }))
      : fallbackGroups;
  }, [normalizedGroupMap, groupsById, fallbackGroups]);

  const [groupIdx, setGroupIdx] = useState<number>(storeGroups.length ? 0 : -1);
  useEffect(() => { if (!isGroups) setGroupIdx(-1); }, [isGroups]);

  const useAllGroups = isGroups && (!storeGroups.length || groupIdx === -1);
  const visible = useMemo(() => {
    if (isGroups) return useAllGroups ? allRowsForStage : allRowsForStage.filter((r) => r.groupIdx === (groupIdx ?? 0));
  ...
  const teamOptions = useMemo(() => {
    ...
    const ids = isGroups && !useAllGroups ? listGroupTeamIds(effectiveStageIdx, effectiveGroupForOptions) : miniPayload.tournament_team_ids ?? [];
  ...
  function ensureRowExists(stageIdxArg: number, round: number, bracket_pos: number) {
    updateMatches(stageIdxArg, (stageRows) => {
      if (stageRows.some((r) => r.stageIdx === stageIdxArg && r.round === round && r.bracket_pos === bracket_pos)) return stageRows;
      return [...stageRows, { 
  ...
  const applyPatch = (target: DraftMatch, patch: Patch) => {
      ...
      if (newR !== currR || newP !== currP) {
        ...
        if (Object.keys(rest).length > 0) {
          updateMatches(effectiveStageIdx, (stageRows) => {
            ...
            const i = next.findIndex((r) => r.stageIdx === effectiveStageIdx && r.round === newR && r.bracket_pos === newP);
            if (i >= 0) {
    ...
    updateMatches(effectiveStageIdx, (stageRows) => {
      ...
      const teamsInGroup = isGroups && !useAllGroups 
  ...
  const addRow = () => {
    if (isKO) {
      ...
      const teamIds = effectiveGroup !== null ? listGroupTeamIds(effectiveStageIdx, effectiveGroup) : miniPayload.tournament_team_ids ?? [];
      const teamCount = teamIds.length;
      const matchesPerDay = Math.floor(teamCount / 2);
      let stageRows = effectiveGroup !== null ? allRowsForStage.filter((r) => r.groupIdx === effectiveGroup) : allRowsForStage;
      ...
      const targetMd = sortedMds.length === 0 ? 1 : matchdayCounts[lastMd] < matchesPerDay ? lastMd : lastMd + 1;
      updateMatches(effectiveStageIdx, (rows) => [...rows, {
  ...
  const regenerateStage = () => {
    ...
    const key = (m: DraftMatch) => {
      ...
      return `RR|G${m.groupIdx ?? -1}|MD${m.matchday ?? 0}|${rrPairKey(m.team_a_id, m.team_b_id)}`;
    };
    ...
    <section className="rounded-lg border border-white/10 bg-slate-950/50 p-3 space-y-3">
      ...
      <header className="flex flex-wrap items-center gap-2">
        <div className="text-white/80 text-sm">
          ...
          {isGroups && (
            <>
              ...
              <select className="ml-2 bg-slate-950 border border-white/15 rounded px-2 py-1 text-white"
                value={useAllGroups ? -1 : groupIdx} onChange={(e) => setGroupIdx(Number(e.target.value))}>
                <option value={-1}>All groups</option>
                {storeGroups.map((g) => (<option key={g.idx} value={g.idx}>{g.name}</option>))}
              </select>
        ...
        <div className="ml-auto flex items-center gap-2">
          <input type="text" className="px-2 py-1.5 rounded border border-white/15 bg-slate-950 text-white text-xs w-56"
            placeholder="Search teams…" value={teamQuery} onChange={(e) => setTeamQuery(e.target.value)} />
          <button className="px-2 py-1.5 rounded border border-cyan-400/30 text-cyan-200 hover:bg-cyan-500/10 text-xs"
            onClick={regenerateStage} title="Rebuild this stage's fixtures (keeps scores/status where possible)">
            Regenerate stage
          </button>
        ...
        <div className="overflow-auto rounded border border-white/10">
          <table className="min-w-full text-sm">
            <thead className="bg-zinc-900/70 text-white">
              <tr>
                {isKO && (<><th className="px-2 py-1 text-left">Round</th><th className="px-2 py-1 text-left">Bracket pos</th></>)}
                    ...
                    <>
                      <tr className="bg-zinc-800/50" key={`md-${md}`}>
                        <td colSpan={6} className="px-4 py-2 font-bold underline text-white text-center">
                          Matchday&nbsp;
                          <input type="number" className="w-16 bg-transparent border-b border-white/50 text-white text-center font-bold focus:outline-none focus:border-white"
                            value={md} min={1} onChange={(e) => {
