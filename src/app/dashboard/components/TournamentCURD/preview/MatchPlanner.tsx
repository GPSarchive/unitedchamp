...
export default function MatchPlanner({
  ...
  compact,
  // NEW: allow parent to trigger seeding before regeneration (used for KO)
  ...
  const swapTeams = (localId: number) => {
    // If there are pending values, swap those; else swap row values into pending for clarity
  ...
  // Apply pending edits of a single row
  const saveRow = (localId: number) => {
  ...
  const regenerateHere = async () => {
    // If current stage is KO, ask parent to reassign seeds (1..N) BEFORE generating the bracket.
    if (isKO && onAutoAssignTeamSeeds) {
      try {
        await Promise.resolve(onAutoAssignTeamSeeds());
      } catch {
        // ignore; generator will still run using existing seeds/order
      }
    }
    ...
    setPending({}); // clear unsaved edits because schedule changed
  };
  ...
  const autoAssignGroupRR = () => {
    ...
    // Put updates into pending (so admin can Save row-by-row or edit further)
                  ...
                  >
                    ...
                    {isKO ? (
                      <>
                        <td className="px-2 py-1">
                          <input
                            ...
                            value={getEff(m as EditableDraftMatch, "round") ?? 1}
                            onChange={(e) =>
                              setPendingFor(lid, {
                        ...
                        <td className="px-2 py-1">
                          <input
                            ...
                            value={getEff(m as EditableDraftMatch, "bracket_pos") ?? 1}
                            onChange={(e) =>
                      ...
                      <td className="px-2 py-1">
                        <input
                          ...
                          value={getEff(m as EditableDraftMatch, "matchday") ?? 1}
                          onChange={(e) =>
                    ...
                    <td className="px-2 py-1">
                      <select
                        ...
                        value={getEff(m as EditableDraftMatch, "team_a_id") ?? ""}
                        onChange={(e) =>
                          setPendingFor(lid, {
                    ...
                    <td className="px-2 py-1">
                      <select
                        className="min-w-48 bg-slate-950 border border-white/15 rounded px-2 py-1 text-white"
                        value={getEff(m as EditableDraftMatch, "team_b_id") ?? ""}
                        onChange={(e) =>
                          setPendingFor(lid, {
                    ...
                    <td className="px-2 py-1">
                      <input
                        ...
                        value={isoToLocalInput(
                          (pending[lid]?.match_date as string | null | undefined) ??
                            m.match_date
                        )}
                        onChange={(e) =>
                          setPendingFor(lid, {
                      ...
                      <div className="flex items-center justify-end gap-2">
                        <label className="inline-flex items-center gap-1 text-xs text-white/80">
                          <input
                            ...
                            onChange={(e) => setRow(lid, { locked: e.target.checked })}
                          />
                          Κλείδωμα
                        </label>

                        {/* NEW: Save button per row */}
                        <button
