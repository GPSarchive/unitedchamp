// app/components/DashboardPageComponents/TournamentCURD/stages/TournamentBracket.tsx
"use client";
...
export default function TournamentBracket({
  ...
  const onMatchClick = (m: Match) => {
    // Only allow finishing matches that have both teams and are not already finished
    if (m.status !== "finished" && m.team_a_id && m.team_b_id) setActive(m);
  };

  // Little helper to show a one-liner header status
    ...
    <div className="relative">
      ...
      {active && (
        <FinishMatchModal
          ...
          onConfirm={async (aScore, bScore) => {
            // derive winner from scores; ties are not allowed for finished KO matches
            if (aScore === bScore) {
              alert("Scores cannot be equal for a finished match.");
              return;
            }
            const winnerId = aScore > bScore ? active.team_a_id! : active.team_b_id!;
