    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 text-zinc-100">
        <div className="grid gap-3">
          <label className="flex flex-col gap-1">
            <span className="text-sm text-zinc-200">Τίτλος</span>
            <input
              className="border border-zinc-700 bg-zinc-900 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              value={title}
          ...
          <label className="flex flex-col gap-1">
            <span className="text-sm text-zinc-200">Κείμενο</span>
            <textarea
              className="border border-zinc-700 bg-zinc-900 rounded-lg px-3 py-2 min-h-[120px] focus:outline-none focus:ring-2 focus:ring-indigo-500"
              value={body}
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              disabled={loading}
              className="px-3 py-2 rounded-lg border border-emerald-600 bg-emerald-600 text-white hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-400 disabled:opacity-60"
            >
              Δημοσίευση τώρα
            </button>
            <button
              ...
              disabled={loading}
              className="px-3 py-2 rounded-lg border border-zinc-700 bg-zinc-800 text-zinc-100 hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-indigo-400 disabled:opacity-60"
            >
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 overflow-hidden">
        {rows.length === 0 && !loading ? (
          <div className="p-5 text-sm text-zinc-300">Δεν υπάρχουν ανακοινώσεις ακόμα.</div>
        ) : null}
        ...
        <div className="min-w-0 flex-1">
          ...
          {!editing ? (
            ...
            <div className="grid gap-2 mt-1">
              <input
                className="border border-zinc-700 bg-zinc-900 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                value={t}
                onChange={(e) => setT(e.target.value)}
              />
              <textarea
                className="border border-zinc-700 bg-zinc-900 rounded px-2 py-2 min-h-[100px] focus:outline-none focus:ring-2 focus:ring-indigo-500"
                value={b}
            ...
            {!editing ? (
              <>
                <button
                  ...
                  className="px-2.5 py-1.5 text-sm rounded-md border border-zinc-700 text-zinc-100 hover:bg-zinc-800"
                >
                  Επεξεργασία
                </button>
                <button
                  onClick={togglePin}
                  className="px-2.5 py-1.5 text-sm rounded-md border border-amber-600 text-amber-100 hover:bg-amber-600/10"
                >
                  {a.pinned ? 'Αφαίρεση καρφίτσας' : 'Καρφίτσωσε'}
                </button>
                <button
                  onClick={togglePublish}
                  className="px-2.5 py-1.5 text-sm rounded-md border border-emerald-600 text-emerald-100 hover:bg-emerald-600/10"
                >
                  {a.status === 'published' ? 'Απόκρυψη (πρόχειρο)' : 'Δημοσίευση'}
                </button>
                <button
                  onClick={() => onDelete(a.id)}
                  className="px-2.5 py-1.5 text-sm rounded-md border border-rose-600 text-rose-200 hover:bg-rose-600/10"
                >
                  Διαγραφή
                </button>
                <Link
              ...
              <>
                <button
                  onClick={save}
                  className="px-2.5 py-1.5 text-sm rounded-md border border-indigo-600 bg-indigo-600 text-white hover:bg-indigo-500"
                >
                  Αποθήκευση
                </button>
                <button
