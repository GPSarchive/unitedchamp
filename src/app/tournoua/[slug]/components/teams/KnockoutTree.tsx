...
export type ModernKnockoutTreeProps = {
  ...
  /** Enable first-round seeding UI */
  editable?: boolean;
  /** Limit which teams are eligible to be placed in this stage */
  eligibleTeamIds?: number[];
  /** Assign a team to a slot (parent updates matches) */
  onAssignSlot?: (matchId: number, slot: "A" | "B", teamId: number | null) => void;
  /** Swap A/B inside a match (optional shortcut) */
  onSwapPair?: (matchId: number) => void;
  /** Bulk-assign for auto-seed; if not provided we’ll call onAssignSlot per slot */
  onBulkAssignFirstRound?: (
    rows: Array<{ matchId: number; team_a_id: number | null; team_b_id: number | null }>
  ) => void;
  /** Clear all first-round picks */
  onClearFirstRound?: () => void;

  /** NEW: when clicking Auto-seed, call this first to (re)assign 1..N seeds.
   * Must return team IDs ordered by ascending seed (1..N). If omitted,
   * we fall back to current seeds found in teamsMap.
   */
...
export default function ModernKnockoutTree({
  ...
  const autoSeedAndPair = async () => {
    // 1) Ask parent to (re)assign seeds and give ordered IDs by seed
    ...
    if (onAutoAssignTeamSeeds) {
      try {
        ...
        // ignore; will fall back to current seeds
      }
    }

    // 2) Build pool from orderedIds or current seeds in teamsMap
    const pool: number[] =
      orderedIds ??
      eligible
    ...
    // 3) Take top S teams and pair (1 vs N), (2 vs N-1), ...
