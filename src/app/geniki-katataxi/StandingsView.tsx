// app/geniki-katataxi/StandingsView.tsx
// The shared server component that renders the Γενική Κατάταξη — used by the live
// route (season from the typed `tournaments.season` field) and by the preview route
// (season derived from `start_date` with the Sept 30 cutoff). All presentation lives
// here so both modes stay pixel-identical; only the `seasonMode`, `basePath` and an
// optional preview banner differ.
...
export interface StandingsViewProps {
  ...
  /** Base route the season tabs link to (e.g. "/geniki-katataxi"). */
  basePath: string;
  /** Requested ?season= value (already un-arrayed). */
  requestedSeason?: string;
  /** Optional banner rendered above the standings (used by the preview tab). */
