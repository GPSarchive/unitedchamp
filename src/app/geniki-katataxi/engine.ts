// app/geniki-katataxi/engine.ts
// The Γενική Κατάταξη points engine — PURE: rows in, standings out. No I/O and
// no Next imports, so it is unit-tested directly (__tests__/points.test.ts).
// points.ts loads the rows (optionally scoped to one season) and caches the
// unscoped result; every rule lives here, every point value in ./rules.
//
...
//
// A tournament's season is its ASSIGNED `tournaments.season` label — never a
// date (plans/seasonal-data-contract.md, principle 2).
...
export type StageRow = { id: number; tournament_id: number; kind: string; ordering: number | null };
export type ParticipationRow = { tournament_id: number; team_id: number; stage_id: number | null };
...
/** Everything the engine needs, as loaded by points.ts (or built by a test). */
export interface StandingsInputs {
  ...
  /** false when the adjustments table could not be read (engine degrades gracefully). */
  adjustmentsAvailable: boolean;
}

export function seasonKey(raw: string | null): string {
  const s = (raw ?? "").trim();
...
/**
 * Compute the standings from already-loaded rows. `onlyLabel` restricts the
 * result to one season (tournaments and adjustments of other seasons are
 * skipped); the loader applies the same scope in its queries, this is the
 * belt to that brace and what the unit tests exercise.
 */
export function computeStandingsFromInputs(
  inputs: StandingsInputs,
  onlyLabel: string | null = null,
): GeneralStandings {
  const { tournaments, stages, participations, matches, adjustments, adjustmentsAvailable } = inputs;
