/* =========================
   ...
   // Option A: strict overlap -> 0
   // Option D: small tolerance -> e.g. 5
   const GAP_MIN = 0;
   ...
   function collapseOverlapsToCompositeEvents(raw: EventInput[]): EventInput[] {
     ...
     for (const [key, arr] of byDay) {
       ...
       const flush = () => {
         ...
         // Compute exact cluster bounds from items
         const startIso = cluster.reduce((m, c) => (c.start < m ? c.start : m), cluster[0].start);
         const endIso = cluster.reduce((m, c) => (c.end > m ? c.end : m), cluster[0].end);
       ...
       for (const e of arr) {
         const overlapThreshold = clusterEnd - GAP_MIN; // tolerance
         if (!cluster.length || e.startMin < overlapThreshold) {
           cluster.push(e);
   ...
   export default function EventCalendar({
     ...
     return (
       <div
         className={`${className ?? ''} overflow-x-hidden border border-black shadow-black shadow-xl bg-zinc-950 bg-gradient-to-br bg-black text-white`}
         style={{
