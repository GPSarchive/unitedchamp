// Server component — mirror of @/app/home/TopPlayersSection that renders the
// editorial top-players cards instead of the original styled ones.
...
async function fetchStatRows() {
  const [
    ...
    supabaseAdmin
      ...
      .select(
        `player_id, total_goals, total_assists, total_mvp, total_matches, primary_team_id,
        player:player_id(id, first_name, last_name, photo, deleted_at),
    ...
    supabaseAdmin
      .from("player_career_stats")
      .select(
        `player_id, total_goals, total_assists, total_best_gk, total_matches, primary_team_id,
        player:player_id(id, first_name, last_name, photo, deleted_at),
        team:primary_team_id(id, name, logo)`
      )
      .gt("total_best_gk", 0)
...
export default async function EditorialTopPlayersSection() {
  const { scorerRows, assisterRows, mvpRows, gkRows, scorersError, assistersError } =
    await fetchStatRows();

  const statsPlayerIds = Array.from(
    new Set([
  ...
  const topScorers =
    scorersError || !scorerRows?.length
      ? []
      : scorerRows.map((stat: any) =>
          buildEmbeddedPlayerEntry(stat, matchesByPlayer.get(stat.player_id)?.size ?? 0)
        );

  const topAssisters =
    assistersError || !assisterRows?.length
      ? []
      : assisterRows.map((stat: any) =>
          buildEmbeddedPlayerEntry(stat, matchesByPlayer.get(stat.player_id)?.size ?? 0)
        );
