...
export async function saveAllStatsAction(formData: FormData) {
  ...
  after(async () => {
    ...
    // The result/winner is final only after progressAfterMatch → rewrite the
    // stored Γενική Κατάταξη rows here, post-response (never throws).
    await refreshActiveSeasonStandings('saveAllStatsAction');
    ...
    // invalidate the tournament/leaderboard/standings surfaces once more.
    try {
