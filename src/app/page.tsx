...
async function fetchTopAssists() {
  return withConsoleTiming('db:top-assists', async () => {
    const { data: statsData, error: statsError } = await supabaseAdmin
      .from('player_statistics')
      .select('player_id, total_goals, total_assists')
    ...
    const topAssists = statsData.map(stat => {
      const player = playersData?.find(p => p.id === stat.player_id);
      const teamId = playerTeamMap.get(stat.player_id);
      const team = teamId ? teamMap.get(teamId) : null;
      const matches = matchesByPlayer.get(stat.player_id)?.size ?? 0;
...
export default async function Home() {
  ...
  const [{ user }, { rawMatches }, { tournaments }, { recentContentCount }, { topScorers }, { videoMatches }, { topAssists }] = await Promise.all([
    fetchSingleUser(),
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden bg-zinc-950">
      ...
      {/* Top Players Section */}
      <TopScorers scorers={topScorers} assistLeaders={topAssists} />
