...
export default function ScorerCard({ player, index }: { player: TopPlayerData; index: number }) {
  const style = rankStyles[index] || rankStyles[3];
  const perGame = player.matches > 0 ? (player.goals / player.matches).toFixed(2) : '0.00';

  return (
    <Card
      customClass={`border ${style.border} bg-[#0c0d10] transition-all duration-500`}
    >
      {/* Warm ambient glow for scorers */}
      <div className="absolute inset-0 bg-[#0c0d10]" />
      <div className={`absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l ${style.accent} opacity-40`} />
        ...
        <div className="flex-1 flex flex-col justify-between py-8 pr-8 pl-6">
          ...
          <div className="flex items-start justify-between w-full">
            ...
            <div className="relative mt-1">
              <div className={`relative w-12 h-14 transform skew-x-[-12deg] flex flex-col items-center justify-center ${style.badge} shadow-sm`}>
          ...
          <div className="relative mt-2">
            <div className="flex items-end gap-3">
              <span className={`text-7xl font-black leading-none tracking-tighter ${style.text}`}>
                {player.goals}
              </span>
          ...
          <div className="flex items-center justify-between border-t border-white/[0.08] pt-5 mt-4">
            <div>
              <div className="text-2xl font-bold text-white tabular-nums">{player.assists}</div>
              <div className="text-[10px] text-white/30 uppercase tracking-[0.1em] font-medium mt-1">Assists</div>
            </div>
            ...
            <div className="w-px h-8 bg-white/[0.08]" />
            <div>
              <div className={`text-2xl font-bold tabular-nums ${style.text}`}>{perGame}</div>
