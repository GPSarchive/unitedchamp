...
export default function AssisterCard({ player, index }: { player: TopPlayerData; index: number }) {
  const style = rankStyles[index] || rankStyles[3];
  const perGame = player.matches > 0 ? (player.assists / player.matches).toFixed(2) : '0.00';

  return (
    <Card
      customClass={`border ${style.border} bg-[#0a0c14] transition-all duration-500`}
    >
      {/* Cool ambient glow for assisters */}
      ...
      <div className="relative flex h-full w-full z-10">
        ...
        <div className="relative w-[42%] h-full overflow-hidden flex-shrink-0 group border-r border-white/5">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#0a0c14]/40 to-[#0a0c14]" />
